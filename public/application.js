...
core.directive('slider',["$interval", function($interval){
	return{
...
core.directive('showbiz',["$interval", function($interval){
	return{
		restrict : 'AE', //attribute element
		replace: true,
		templateUrl: '/modules/core/views/showbiz.view.html',
		scope: {
			data: '='
		},
		controller: ["$scope", function($scope){
			$scope.leftEntries = [];
			$scope.entries = [];
			$scope.rightEntries = $scope.data;


			$scope.left = function(){
				if($scope.rightEntries.length > 0){
					$scope.leftEntries.push($scope.entries.shift());
					$scope.entries.push($scope.rightEntries.shift());
				}
			}

			$scope.right = function(){
				if($scope.leftEntries.length > 0) {
					$scope.entries.unshift($scope.leftEntries.pop());
					$scope.rightEntries.unshift($scope.entries.pop());
				}
			}
...
core.controller('HomeController',
	["$scope", "Authentication", "$timeout", function($scope, Authentication,$timeout) {
		// This provides Authentication context.
		$scope.authentication = Authentication;
